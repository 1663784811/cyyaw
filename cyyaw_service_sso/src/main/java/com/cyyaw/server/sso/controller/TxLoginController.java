...
import com.cyyaw.server.sso.service.TxService;
...
@RequestMapping("/login/tx/admin/")
@RestController
public class TxLoginController {
     ...
     */
    @PostMapping("/login")
    public BaseResult login(@RequestBody JSONObject json, HttpServletRequest request) {
        String account = json.getString("account");
        String password = json.getString("password");
        String tokenstr = request.getHeader("token");
        String msg;
        if (!StringUtilWHY.isEmpty(account) && !StringUtilWHY.isEmpty(password)) {
            BaseResult baseres = txService.findAdminByAccount(account);
    ...
    public BaseResult storeRegister(@RequestBody JSONObject json) {
        ...
        if (!StringUtilWHY.isEmpty(account) && !StringUtilWHY.isEmpty(password)) {
            ...
            json.put("salt", salt);
            return txService.saveAdmin(json);
        } else {
